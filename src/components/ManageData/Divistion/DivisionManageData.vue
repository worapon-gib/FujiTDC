    ...
    <v-row class="text-center mt-8" align="center" justify="center">
      <v-col cols="11" align="right">
        <v-btn
          ...
          @click="openDialogAddEditDivision('ADD', 'no-item')"
        >
      ...
      <v-col cols="11" class="height_table_manage_data" align="center">
        ...
        >
          <!-- Data Table -->
          <v-simple-table v-if="item_list_division.length > 0">
            <template v-slot:default>
              ...
              <tbody>
                <tr v-for="(item, index) in item_list_division" :key="index">
                  <td class="text-center pointer pt-3">
                    <span class="primary--text" style="cursor: pointer">
                      {{ item_list_division.indexOf(item) + 1 + ". " }}
                    </span>
                  </td>
                  ...
                  <td class="text-center pt-3">
                    <v-icon
                      ...
                      color="green"
                      size="28px"
                       @click="openDialogAddEditDivision('EDIT', item)"
                    >
                      mdi-pencil-outline
                    </v-icon>
                    <v-icon class="pl-4" color="red" size="28px" @click="openDialogDeleteDivision(item)">
                      mdi-delete-outline
                    </v-icon>
                  </td>
                </tr>
      ...
      <v-col cols="11" align="left">
        <h4 class="primary--text font-weight-bold">
          {{ item_list_division.length }} รายการ
        </h4>
      </v-col>
    </v-row>
  </div>
...
export default {
  ...
  methods: {
   openDialogAddEditDivision(mode, item) {
      this.$emit("openDialogAddEditDivision", mode, item);
    },
    openDialogDeleteDivision(item) {
      this.$emit("openDialogDeleteDivision", item);
    },
