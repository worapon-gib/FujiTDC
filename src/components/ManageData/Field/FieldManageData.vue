    ...
    <v-row class="text-center mt-8" align="center" justify="center">
      <v-col cols="11" align="right">
        <v-btn
          ...
          @click="openDialogAddEditField('ADD', 'no-item')"
        >
      ...
      <v-col cols="11" class="height_table_manage_data" align="center">
        ...
        >
          <!-- Data Table -->
          <v-simple-table>
            <template v-slot:default :loading="loading">
              <thead>
                <tr>
                  <th
              ...
              <tbody>
                <tr v-for="(item, index) in item_list_field" :key="index">
                  <td class="text-center pointer pt-3">
                    <span class="primary--text" style="cursor: pointer">
                      {{ item_list_field.indexOf(item) + 1 + ". " }}
                    </span>
                  </td>
                  ...
                  <td class="text-center pt-3">
                    <v-icon
                      ...
                      color="green"
                      size="28px"
                      @click="openDialogAddEditField('EDIT', item)"
                    >
                      mdi-pencil-outline
                    </v-icon>
                    <v-icon
                      ...
                      color="red"
                      size="28px"
                      @click="openDialogDeleteField(item)"
                    >
                      mdi-delete-outline
                    </v-icon>
                  </td>
...
export default {
  ...
  methods: {
    openDialogAddEditField(mode, item) {
      this.$emit("openDialogAddEditField", mode, item);
    },
    openDialogDeleteField(item) {
      this.$emit("openDialogDeleteField", item);
    },
